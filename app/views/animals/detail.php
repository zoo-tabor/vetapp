...
<div class="card" style="margin-top: 20px;">
    <div class="card-header">
        <h2>Hmotnost</h2>
        <?php if ($canEdit): ?>
            <button type="button" class="btn btn-sm btn-outline" onclick="toggleWeightForm()" id="weightToggleBtn">
                + Přidat měření
            </button>
        <?php endif; ?>
    </div>
        ...
        <div id="weightForm" style="display: none; background: #f8f9fa; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr 2fr auto; gap: 12px; align-items: end;">
                <div>
                    ...
                    <input type="number" id="weightInput" step="0.01" min="0.01" placeholder="0.00"
                           class="form-control" style="width: 100%;">
                </div>
                <div>
                    <label style="display: block; font-size: 0.85em; color: #666; margin-bottom: 4px;">Datum *</label>
                    <input type="date" id="weightDate" value="<?= date('Y-m-d') ?>"
                           class="form-control" style="width: 100%;">
                </div>
                <div>
                    <label style="display: block; font-size: 0.85em; color: #666; margin-bottom: 4px;">Poznámka</label>
                    <input type="text" id="weightNotes" placeholder="Volitelná poznámka"
                           class="form-control" style="width: 100%;">
                </div>
                <div>
...
function toggleTransferLocation() {
    ...
    // Datum úmrtí ukázat jen u stavu „Uhynulo".
...
function saveWeight() {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (tbody) {
                // Remove bold from previous first row
                ...
                // No table existed yet — reload to show it properly
                location.reload();
                return;
            }
